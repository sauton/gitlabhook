...
class WebHook
{
    ...
    const GITLAB_BASE_HTML = '/var/www/html';
                            ...
                            if (in_array($event, $this->events)) {
                                if (is_dir($gitpath)) {
                                    if (($body->object_kind == 'push' && $refbranch == $body->ref) ||
                                        ...
                                        if (!$return) {
                                            $result = true;
                                            if (is_dir(self::GITLAB_BASE_HTML . '/' . $repo)) {
                                                if (is_dir(self::GITLAB_BASE_DIR . '/' . $repo . '/code/theme')) {
                                                    $theme_srcgit = self::GITLAB_BASE_DIR . '/' . $repo . '/code/theme';
                                                    $theme_srcwww = self::GITLAB_BASE_HTML . '/' . $repo . '/wp-content/themes/' . $repo;
                                                    ...
                                                    if (!$ref) {
                                                        ...
                                                        $output .= 'Cant coppy ' . $theme_srcgit . ' to ' . $theme_srcwww . '<br>';
                                                    }
                                                } else {
                                                    $output .= self::GITLAB_BASE_DIR . '/' . $repo . '/code/theme' . " is not folder <br>";
                                                }
                                                if (is_dir(self::GITLAB_BASE_DIR . '/' . $repo . '/code/plugin')) {
                                                    $plugin_srcgit = self::GITLAB_BASE_DIR . '/' . $repo . '/code/plugin/*';
                                                    $plugin_srcwww = self::GITLAB_BASE_HTML . '/' . $repo . '/wp-content/plugins/';
                                                    ...
                                                    if (!$ref) {
                                                        ...
                                                        $output .= 'Cant coppy ' . $plugin_srcgit . ' to ' . $plugin_srcwww . '<br>';
                                                    }

                                                } else {
                                                    $output .= self::GITLAB_BASE_DIR . '/' . $repo . '/plugin' . " is not folder <br>";
                                                }
                                            } else {
                                                $output .= self::GITLAB_BASE_HTML . '/' . $repo . ' is not folder <br>';
                                            }
                                            if (empty($output)) {
